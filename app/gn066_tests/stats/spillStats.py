...
def spill_stats(spills_df, df, runs, run_tests=[1, 2]):
    ...
    if 1 in run_tests:
        ...
        test_1_results = spills_df.apply(
            lambda row: test1.test_1(row, df), axis=1, result_type="expand"
        )
        spills_df[
            [
    ...
    if 2 in run_tests:
        ...
        spills_df["Test 2 Status"] = spills_df.apply(
            lambda row: test2.test_2(row, df, results_ts), axis=1
        )
    ...
    years = []
    groupedbyDate = spills_df.groupby(
        pd.Grouper(key="Start of Spill (absolute)", freq="Y")
    )
    ...
    for yrindex, (yr, yr_grp) in enumerate(groupedbyDate):
        ...
        for i, (run, run_grp) in enumerate(yr_grp.groupby(["RUN"])):
            spill_count[i * 3][yrindex] = (
                run_grp["Classification"].value_counts().get("Unsatisfactory", 0)
            )
            spill_count[(i * 3) + 1][yrindex] = (
                run_grp["Classification"].value_counts().get("Substandard", 0)
            )
            spill_count[(i * 3) + 2][yrindex] = (
                run_grp["Classification"].value_counts().get("Satisfactory", 0)
            )
